...
const License: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        const [licensesResponse, authoritiesResponse, regionsResponse, quantitiesResponse, statusesResponse] = await Promise.all([
          axios.get("http://127.0.0.1:8000/api/licenses"),
          axios.get("http://127.0.0.1:8000/api/issuing"),
          axios.get("http://127.0.0.1:8000/api/regions"),
          axios.get("http://127.0.0.1:8000/api/quantities"),
  ...
  const onFinish = async (values: any) => {
    try {
      const response = await axios.post("http://127.0.0.1:8000/api/licenses", values);
  ...
  const handleDelete = async (key: React.Key) => {
    try {
      await axios.delete(`http://127.0.0.1:8000/api/licenses/${key}`);
